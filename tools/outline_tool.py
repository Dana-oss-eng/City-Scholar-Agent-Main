...
def build_outline_sections(topic: str, comparison: MultiPaperComparison) -> list[ReviewOutlineSection]:
    ...
    sections = [
        build_section(
            title='1. 研究背景与问题提出',
            bullets=[
                f"围绕「{topic}」这一主题，梳理城市研究领域的核心关切与现实需求。",
                *comparison.common_themes,
            ],
            default_bullet='阐述研究主题的现实背景与学术意义，明确综述的核心问题。',
        ),
        build_section(
            title='2. 研究对象与案例场景',
            bullets=[
                f'《{item.file_name}》：{item.research_object[:120] if len(item.research_object) > 120 else item.research_object}'
                for item in comparison.paper_summaries
            ],
            default_bullet='概括不同论文关注的城市、区域、社区或系统类型及其特征。',
        ),
        build_section(
            title='3. 研究方法与数据来源',
            bullets=comparison.method_comparison[:4] + comparison.data_comparison[:4],
            default_bullet='对比各论文在方法选择和数据使用上的异同与优劣。',
        ),
        build_section(
            title='4. 主要发现与研究进展',
            bullets=comparison.finding_comparison[:4],
            default_bullet='综合各论文的核心发现，识别共识性结论与存在的分歧。',
        ),
        build_section(
            title='5. 对城市治理、规划或安全的启示',
            bullets=comparison.integrated_implications[:4],
            default_bullet='从研究发现中提炼对城市治理实践具有指导意义的建议。',
        ),
        build_section(
            title='6. 研究局限与未来方向',
            bullets=[
                f'《{item.file_name}》：{item.limitations[:120] if len(item.limitations) > 120 else item.limitations}'
                for item in comparison.paper_summaries[:4]
            ],
            default_bullet='从样本覆盖、方法局限、数据约束和外部推广性等角度总结不足，并展望后续研究方向。',
        ),
    ]
    return sections
