...
def extract_compare_keywords(text: str) -> list[str]:
    ...
    english_terms = re.findall(r"[a-z]{4,}", normalized_text)  # \u81f3\u5c114\u5b57\u6bcd\u51cf\u5c11\u566a\u58f0
...
def collect_common_themes(analyses: list[StructuredPaperAnalysis], max_items: int = 4) -> list[str]:
    '''Extract common themes from multiple papers, prioritizing meaningful research themes.

    Args:
        analyses: List of structured paper analyses.
        max_items: Maximum number of common themes to return.
    Returns:
    ...
    for analysis in analyses:
        ...
        for pattern in MEANINGFUL_THEME_PATTERNS:
            if re.search(pattern, merged_text, re.IGNORECASE):
                theme_counter[pattern] += 1

    # 取至少 2 篇论文中出现的主题
    meaningful_themes = [t for t, c in theme_counter.items() if c >= 2]
    # 去重：去除较短的主题词（被长词包含的）
    deduped: list[str] = []
    for theme in sorted(meaningful_themes, key=lambda t: -len(t)):
        if not any(theme != existing and theme in existing for existing in deduped):
    ...
    if deduped:
        ...
        display = [translations.get(t, t) for t in deduped[:max_items]]
        return [f'多篇论文共同关注「{t}」相关议题。' for t in display]

    # 第二步：回退到词频统计（使用扩展停用词表过滤后）
    ...
    common_terms = [term for term, count in counter.items()
                    if count >= 2 and len(term) >= 4 and term not in COMPARE_STOP_WORDS][:max_items]
    if not common_terms:
        return ['几篇论文分别从韧性评估、空间分析和治理策略等角度切入，共同关注城市可持续发展问题。']
