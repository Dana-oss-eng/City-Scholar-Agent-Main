...
FIELD_RULES = {
    "research_question": {
        ...
        "default": "未能从文本中自动识别出明确的研究问题表述。学术论文的研究问题通常出现在摘要末尾或引言部分，建议人工查阅原文的 Abstract 和 Introduction 章节。",
    },
    "research_object": {
        "label": "研究对象",
        "keywords": ["研究对象", "对象", "样本", "案例", "城市", "社区", "居民", "区域", "街区", "城市群"],
        "default": "未能自动识别出明确的研究对象描述。研究对象通常包括城市名称、区域范围、样本群体或案例类型，建议查阅原文的方法与数据部分。",
    },
    "methods": {
        "label": "方法",
        "keywords": ["方法", "采用", "构建", "模型", "访谈", "问卷", "实证", "回归", "评价", "分析方法"],
        "default": "未能自动识别出研究方法。学术论文的方法论通常出现在 Methods / Methodology 或「研究设计」章节中，建议人工查阅原文。",
    },
    "data_source": {
        "label": "数据来源",
        "keywords": ["数据", "数据来源", "样本", "年鉴", "问卷", "访谈", "遥感", "POI", "统计", "文本数据"],
        "default": "未能自动识别出数据来源。数据说明通常出现在 Methods 或 Data 章节，建议查阅原文的数据描述部分。",
    },
    "key_findings": {
        "label": "主要结论",
        "keywords": ["结论", "发现", "结果", "表明", "说明", "影响", "显著", "提升", "降低"],
        "default": "未能自动识别出主要结论。核心发现通常出现在 Results、Findings 或 Conclusion 章节，建议查阅原文的结果与分析部分。",
    },
    "limitations": {
        "label": "局限性",
        "keywords": ["局限", "不足", "受限", "限制", "未来", "仍需", "但", "然而"],
        "default": "未能自动识别出局限性讨论。研究局限性通常出现在 Discussion 或 Conclusion 末尾段落，建议人工查阅。",
    },
    "implications": {
        "label": "对城市治理/规划/安全的启示",
        "keywords": ["启示", "建议", "治理", "规划", "安全", "政策", "管理", "优化", "决策"],
        "default": "未能自动识别出政策启示。实践建议通常出现在 Conclusion 或 Policy Implications 部分，建议人工查阅原文末尾章节。",
    },
}
...
def is_reference_like_sentence(sentence: str) -> bool:
    ...
    if len(normalized_sentence) < 8:
        return True
    if lowered_sentence.startswith(("doi", "http", "www", "copyright", "article info", "received:", "accepted:", "published:")):
        return True
    if re.search(r"\bvol\b|\bno\b|\bpp\b|\bjournal\b|\buniversity\b|\bpress\b|\bdoi\b|\belsevier\b|\bspringer\b|\btaylor\b|\bfrancis\b|\bwiley\b|\bsage\b", lowered_sentence):
        return True
    if " et al" in lowered_sentence and len(normalized_sentence) < 60:
        return True
    # 过滤纯出版信息行
    if re.match(r"^[&\sA-Za-z0-9(),.\-]+$", normalized_sentence) and len(normalized_sentence) < 40:
        return True
...
def truncate_sentence(text: str, max_length: int = 120) -> str:
    ...
    # 在 max_length 范围内寻找最近的句子边界
    truncated = cleaned_text[:max_length]
    # 尝试在最后一个 . 、；或空格处截断，避免截断单词
    for boundary_char in [". ", "? ", "! ", "；", "。", "，", " "]:
        last_pos = truncated.rfind(boundary_char)
        if last_pos > max_length * 0.5:  # 至少保留一半长度
            return truncated[:last_pos].rstrip() + "..."
