...
│
├── data/                           # 数据目录
│   └── raw_papers/                 # 原始 PDF 论文（约 40 篇城市研究论文）
│
├── outputs/                        # 输出目录
│   ├── workflow_report.md          # 工作流 Markdown 报告示例
│   ├── 城市研究.md                  # 城市研究综述报告示例
│   ├── entity_graph.png            # 实体共现图谱（PNG 图片）
│   └── knowledge_graph_demo.html   # 知识图谱交互式可视化示例
│
...
| 组件 | 文件 | 说明 |
|------|------|------|
| 论文加载 | `loader.py` | 扫描 `data/raw_papers/` 下的 PDF 文件 |
...
项目通过 `App.py` 中的环境变量注入 API Key，支持两种方式：

**方式一：直接修改 App.py（默认）**

`App.py` 启动时会自动设置以下环境变量（已内嵌默认 Key）：
```python
...
> **说明**：对话与分析优先使用 DeepSeek，向量嵌入仅使用 DashScope。至少需要配置一个对话模型和一个嵌入模型。LLM 客户端均使用原生 `urllib` 直接调用 API，无需安装额外的 SDK。

### 4. 放入论文 PDF

将待分析的 PDF 论文放入 `data/raw_papers/` 目录。项目已预置约 40 篇城市韧性、城市治理相关的高质量论文。
...
Loader Demo
论文目录：D:\LLM Agent\City-Scholar-Agent-Main\data\raw_papers
发现 PDF 数量：40
...
├──────────────────────────────────────────────────────────────┤
│  data/raw_papers/              │  outputs/                    │
│  (~40 篇城市研究 PDF)           │  (.md / .html / .png)        │
└──────────────────────────────────────────────────────────────┘
```
